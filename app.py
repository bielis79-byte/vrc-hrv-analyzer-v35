...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v3.5", layout="wide")
...
# Para evitar que el grafo HVG bloquee la app en ventanas largas.
# Si una ventana tiene más RRi que este límite, se toma una muestra uniforme para el HVG.
MAX_HVG_NODES_DEFAULT = 900
...
st.title("VRC / HRV RRi Analyzer Pro v3.5")
st.caption("Segmentación flexible, análisis HRV, HVG, Poincaré, gráficas y exportación. Basal + E1-E6 + R1-R3. Ventanas de duración libre.")

if "windows" not in st.session_state:
    # Ventanas por defecto orientativas. No son obligatoriamente de 5 min:
    # puedes cambiar inicio y fin libremente desde la pestaña Segmentación.
    ...
    # Si venías de una versión antigua con Ejercicio/Recuperación únicos,
    # se convierten automáticamente a E1-E6 y R1-R3 dividiendo la ventana.
    old_w = st.session_state.windows.copy()
    if "Ejercicio" in old_w or "Recuperación" in old_w:
        new_w = {}
        if "Basal" in old_w:
            new_w["Basal"] = old_w["Basal"]
        else:
            new_w["Basal"] = [0.0, 300.0]

        if "Ejercicio" in old_w:
            s, e = old_w["Ejercicio"]
            step = (e - s) / 6.0 if e > s else 300.0
            for i in range(1, 7):
                new_w[f"E{i}"] = [s + (i - 1) * step, s + i * step]

        if "Recuperación" in old_w:
            s, e = old_w["Recuperación"]
            step = (e - s) / 3.0 if e > s else 300.0
            for i in range(1, 4):
                new_w[f"R{i}"] = [s + (i - 1) * step, s + i * step]

        for ph in PHASES:
            new_w.setdefault(ph, st.session_state.windows.get(ph, [0.0, 300.0]))
...
max_hvg_nodes = st.sidebar.number_input("Máximo RRi para HVG por ventana", min_value=100, max_value=5000, value=MAX_HVG_NODES_DEFAULT, step=100, help="Para ventanas largas, el HVG se calcula sobre una muestra uniforme para que la app no se bloquee.")
...
with tab1:
    ...
    active = st.selectbox("Fase a ajustar con el ratón", PHASES)
    ...
    st.info("Las ventanas no tienen duración fija: puedes hacerlas de 1 min, 5 min, 10 min o el intervalo que necesites. Sólo deben quedar dentro del registro y contener al menos 30 RRi para calcularse.")

    if st.button("Crear ventanas automáticas sobre todo el registro"):
        # Divide todo el registro en 10 ventanas consecutivas: Basal + E1-E6 + R1-R3.
        step = t_max / len(PHASES) if t_max > 0 else 300.0
        st.session_state.windows = {ph: [i * step, (i + 1) * step] for i, ph in enumerate(PHASES)}
        st.success("Ventanas automáticas creadas sobre toda la duración disponible. Puedes ajustarlas después manualmente.")
...
segment_info = pd.DataFrame([
    {
        "Fase": name,
        "RRi": int(len(seg)),
        "Duración_min": round((st.session_state.windows[name][1] - st.session_state.windows[name][0]) / 60, 2),
        "Válida_para_HRV": bool(len(seg) >= 30),
    }
...
valid_phase_names = [name for name in PHASES if valid_segments.get(name, False)]

if len(valid_phase_names) == 0:
    st.warning("Ninguna ventana tiene al menos 30 RRi. Ajusta los tiempos de inicio/fin dentro del registro real.")
elif not all(valid_segments.values()):
    bad = [name for name, ok in valid_segments.items() if not ok]
    st.warning(
        "Estas ventanas no se calcularán porque tienen menos de 30 RRi o están fuera del registro: " + ", ".join(bad) +
        ". El resto de ventanas válidas sí se calcularán."
    )


def prepare_hvg_rr(seg, max_nodes):
    seg = np.asarray(seg, dtype=float)
    if len(seg) <= max_nodes:
        return seg, False
    idx = np.linspace(0, len(seg) - 1, int(max_nodes)).astype(int)
    return seg[idx], True


@st.cache_data(show_spinner=False)
def cached_calculate(rr_tuple, windows_tuple, max_nodes):
    ...
    for name, (s, e) in windows.items():
        ...
        # HRV lineal, PSD, DFA, entropías, RQA y MSE sobre la ventana completa.
        rr_ms = seg * 1000
        out = {}
        out.update(time_metrics(seg))
        out.update(psd_metrics(seg))
        a1, a2 = dfa_calc(rr_ms)
        out["DFA_alpha1"] = a1
        out["DFA_alpha2"] = a2
        out["ApEn"] = apen_calc(rr_ms)
        ...
        # HVG sobre ventana completa si es corta; si es larga, muestra uniforme para no bloquear Streamlit.
        seg_hvg, sampled = prepare_hvg_rr(seg, max_nodes)
        hvg_out, _ = hvg_metrics(seg_hvg)
        hvg_out["HVG_sampled"] = sampled
        hvg_out["HVG_original_RRi"] = int(len(seg))
        hvg_out["HVG_used_RRi"] = int(len(seg_hvg))
        out.update(hvg_out)
        results[name] = out

    df = pd.DataFrame(results).T
    if not df.empty:
        df = df.reindex([ph for ph in PHASES if ph in df.index])
    return df


windows_tuple = tuple((k, tuple(st.session_state.windows[k])) for k in PHASES)

with st.spinner("Calculando métricas de las ventanas válidas..."):
    metrics_df = cached_calculate(tuple(rr.tolist()), windows_tuple, int(max_hvg_nodes))

graphs = {}

for name in PHASES:
    seg = segments.get(name, np.array([]))
    if valid_segments.get(name, False):
        seg_hvg, _ = prepare_hvg_rr(seg, int(max_hvg_nodes))
        _, G = hvg_metrics(seg_hvg)
        graphs[name] = G
...
with tab2:
    ...
    st.markdown("### Estado de las ventanas")
    st.dataframe(segment_info, use_container_width=True)

    if metrics_df.empty:
        st.info("Ajusta ventanas válidas. Cada ventana debe estar dentro del registro y contener al menos 30 RRi.")
    else:
...
with tab3:
    ...
    if metrics_df.empty:
        ...
        if "HVG_sampled" in metrics_df.columns and metrics_df["HVG_sampled"].astype(bool).any():
            st.warning("Algunas ventanas largas usan una muestra uniforme para el HVG. Revisa HVG_original_RRi y HVG_used_RRi.")
...
with tab4:
    ...
    if metrics_df.empty:
        ...
        if modo_grafica == "Todas las fases válidas":
            ...
            st.markdown("### Parrilla 6 paneles: fases válidas")
            st.image(tmp1.name)
